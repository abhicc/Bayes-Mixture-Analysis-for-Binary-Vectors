#==========================================================================================
# Dataset: Simulated complete dataset . This is the MASTER code (2nd sampler)
...
update_weight=function(reps)
{
  for(j in 1:k){weight_pars[reps,j]=(rho/k)+sum(idmat[,reps-1]==j)}
  weight_mat[reps,]=rdirichlet(1,weight_pars[reps,])
  return(list(w_m=weight_mat[reps,],w_p=weight_pars[reps,]))
}
update_id=function(reps)
{
  ...
  for(i in 1:nrow(d2))
  {
    ...
    {
      index=(((j-1)*p)+1):(j*p)
      fs[j]=ifelse(is.nan(f(alphas[j],d2[i,],mu_mat[reps,index]))==TRUE,0,
                   f(alphas[j],d2[i,],mu_mat[reps,index]))
    }
    if (sum(weight_mat[reps,]*fs)==0)
    {mat[list_identical[[i]],]=matrix(rep(1/k,k),nrow=length(list_identical[[i]]),ncol=k,byrow=TRUE)} else
    {mat[list_identical[[i]],]=matrix((weight_mat[reps,]*fs)/sum(weight_mat[reps,]*fs),
                                      nrow=length(list_identical[[i]]),ncol=k,byrow=TRUE)}

  }
  ids=apply(mat,1,function(x) sample(c(1:k),1,replace=TRUE,prob=x))
  return(ids)
}
#==========================================================================================
...
d2=train_dist %>% select(-c(n,m)) %>% as.matrix;list_identical=list()
for(i in 1:nrow(d2)){list_identical[[i]]=which(apply(train_original,1,
                        function(x) identical(as.vector(x),as.vector(d2[i,]))))}
weight_mat=matrix(nrow=nreps+1,ncol=k);weight_pars=matrix(nrow=nreps+1,ncol=k)
...
#==========================================================================================
weight_pars[1,]=rep(rho/k,k);weight_mat[1,]=rdirichlet(1,weight_pars[1,])
...
for(reps in 2:(nreps+1))
{
  ...
  weights=update_weight(reps)
  weight_mat[reps,]=weights$w_m;weight_pars[reps,]=weights$w_p
...
#==========================================================================================
save.image(file="complete01_S2.RData")
#==========================================================================================